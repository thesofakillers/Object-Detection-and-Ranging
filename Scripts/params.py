...
# in general, larger = better performance, but potentially slower
...
# reduce to improve speed, set to 0 for variable number
...
try:
    ...
    # -- requires extra modules and non-free build flag
    DETECTOR = cv2.xfeatures2d.SIFT_create(
        nfeatures=BOW_fixed_feature_per_image_to_use)
    ...
    DETECTOR = cv2.ORB_create(
        nfeatures=BOW_fixed_feature_per_image_to_use)  # check these params
    ...
    if (not(BOW_use_ORB_always)):
        ...
        # set this as a flag we can check later which data type to uses
        BOW_use_ORB_always = True

print("For BOW - features in use are: ",
      DETECTOR.__class__(), "(ignore for HOG)")
